...
def main():
    ...
    datasets=[load_dataset_cfg(k,v,args.single_class) for k,v in cfg["datasets"].items()]
    ...
    for m in models:
        if not m["script_path"]:
            raise RuntimeError(f"Model {m['config_name']} missing script_path in config. Config: {m}")

    params=cfg.get("eval_params",{})


    base_out=Path("/home/src/eval/coco_test_metrics")
    ...
    for ds in datasets:
        ...
        for m in models:
            ...
            metrics=eval_model_on_dataset_coco(m["weights"], ds, params, coco_cache, pred_cache, m["script_path"], m["config_name"])
